...
use crate::{Blockchain, BlockchainBehaviour, KVStorage, RocksDb, UTXOSet, Commands, Messages, Block, Wallets, Transaction};
...
impl<T: KVStorage> Node<T> {
    ...
    // 异步转账并挖矿
    async fn transfer_and_mining(&mut self, from: &str, to: &str, amount: i32) -> Result<()> {
        // 首先完成转账
                    ...
                    match command {
                        Ok(cmd) => match cmd {
                            ...
                            Commands::Trans{from, to, amount} => {
                                self.transfer_and_mining(&from, &to, amount.parse::<i32>().unwrap()).await?;
                            },
                        },
                        Err(e) => {
